 ...
 */
public class Get_LastData_Util {
    ...
    Context con;
    public Get_LastData_Util(Context con,String ask,HashMap<String,Object> list)
    {
        this.con=con;
        this.ask=ask;
        this.list=list;
    ...
    public void connectioning()
    {
        ...
        switch ((Integer)list.get("Class"))
        {
            case 0:
                networkAsyncTask.execute("NETWORK_POST_JSON"
                        ,list.get("phone").toString()
                        ,list.get("password").toString()
                        ,list.get("path").toString()
                        ,list.get("Class").toString()
                        ,list.get("video").toString());
    ...
    class NetworkAsyncTask extends AsyncTask<String, Integer, Map<String, Object>> {
        ...
        protected Map<String, Object> doInBackground(String... params) {
            ...
            String phone=null , password =null, path=null,video=null,Class=null;
            switch (Integer.getInteger(params[4])) {

                case 0:
               phone = params[1];
                    password = params[2];
                    path = params[3];
                    video = "videofile" + params[4];
                    break;
                case 1:
                    phone = params[1];
                    password = params[2];
            ...
            try {
                if (NETWORK_GET.equals(action)) {
                    //发送GET请求
                    url = new URL("http://192.168.1.112:1103/ ?name=孙群&age=27");
                    ...
                    //用POST发送键值对数据
                    url = new URL("http://192.168.1.112:1103/reg/phone");
                    conn = (HttpURLConnection) url.openConnection();
                    ...
                    requestBody = new String("PhoneNumber=1202020202").getBytes("UTF-8");
                    //将请求体写入到conn的输出流中
                    ...
                    //  jsonObject.put("phonenumber=", 1543234324);
                  switch (Integer.getInteger(Class))
                  {
                      case 0:
                          break;
                      case 1:
                          Register(conn,"phone="+phone+"&password="+password,os);
                          //注册
                          break;
                  }
                    //记得调用输出流的flush方法
                    ...
                    String strData=new String(responseBody);
                    Looper.prepare();
                   Toast.makeText(con,strData,Toast.LENGTH_LONG).show();
                    Looper.loop();
                }
    ...
    public void Register(HttpURLConnection con,String data,OutputStream out)
    {
